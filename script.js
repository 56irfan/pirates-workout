...
function showStep(n) {
  ...
  if(s){ s.style.display='flex'; s.style.animation='fadeSlideUp 0.35s cubic-bezier(.34,1.56,.64,1)'; }
}

// ── Option card highlight ──
function obSelect(groupClass, activeEl){
  document.querySelectorAll('.'+groupClass).forEach(el=>{
    el.style.borderColor='rgba(255,255,255,0.1)';
    el.style.background='rgba(255,255,255,0.05)';
  });
  if(activeEl){
    activeEl.style.borderColor='var(--gold)';
    activeEl.style.background='rgba(255,183,3,0.08)';
  }
}

// ── Character select ──
function obSetChar(c){ obChar=c; }

// ── Goal select ──
function obSetGoal(g){ obGoal=g; }

// ── Focus area select ──
function obSetFocus(f){ obFocus=f; }
...
function adjYear(d){
  ...
  document.getElementById('ob-age-show').textContent='Age: '+age+' yrs · '+age+' বছর';
}

// ── Name save → go to body info ──
function obSaveName(){
  const v=document.getElementById('ob-name').value.trim();
  if(!v){ 
    const inp=document.getElementById('ob-name');
    inp.style.borderColor='var(--red)';
    inp.placeholder='নাম দিন... / Enter name';
    setTimeout(()=>{ inp.style.borderColor='rgba(255,183,3,0.35)'; inp.placeholder='Captain...'; },1500);
    return;
  }
  S.playerName=v;
  showStep(6);
...
function updateObBMI(){
  ...
  let label='Normal ✓',color='#22c55e';
  if(bmiVal<18.5){label='Underweight';color='#023E8A';}
  else if(bmiVal>=25&&bmiVal<30){label='Overweight';color='#FFB703';}
  else if(bmiVal>=30){label='Obese';color='#D62828';}
  const el=document.getElementById('ob-bmi-show');
  if(el){el.textContent='BMI: '+bmiVal+' — '+label;el.style.color=color;}
}

// ── Google login during onboarding ──
async function obGoogleLogin(){
  try{
    await fbSignIn();
    // fbSignIn triggers onAuthStateChanged which calls obFinish via initApp
  }catch(e){notif('Login failed. Try again.','#D62828');}
}

// ── Finish with full data ──
async function obFinish(){
  ...
  render();
  notif('Welcome, '+S.playerName+'!','#FFB703');
}

// ── Guest finish (skip login) ──
function obFinishGuest(){
  const totalInch=(obFeet*12)+obInch;
  S.weight=obWeight;
  S.height=Math.round(totalInch*2.54);
  S.heightFeet=obFeet;
